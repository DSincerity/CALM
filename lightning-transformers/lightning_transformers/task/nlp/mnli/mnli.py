...
class mnli(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, split, dataset, dataset_2=None):
        import pandas as pd
        idx = dataset['idx']
        premise = [f'Premise: {p}' for p in dataset['premise']]
        hypothesis = [f'Hypothesis: {p}' for p in dataset['hypothesis']]
        ...
        if dataset_2 is not None:
            ...
            premise += [f'Premise: {p}' for p in dataset_2['premise']]
            hypothesis += [f'Hypothesis: {p}' for p in dataset_2['hypothesis']]
